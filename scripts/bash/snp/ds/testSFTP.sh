...
export LOGFILE="/tmp/sftptest_$$.log"
...
if [ $AUTH = "BA" ]; then
  /u01/app/cm/scripts/bash/snp/ds/testSFTPPasswd.sh $HOSTNAME $PORT $USERNAME $PASSWORD > $LOGFILE 2>&1
elif [ $AUTH = "PK" ]; then
  /u01/app/cm/scripts/bash/snp/ds/testSFTPPvtkey.sh $HOSTNAME $PORT $USERNAME $PVTKEY_FILE $PASSPHRASE  > $LOGFILE 2>&1
fi
